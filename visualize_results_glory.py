"""
Visualization module for Glory-mode (no SST/SSS/SLA) inference results.

Mirrors visualize_results.py exactly, just without surface-variable content.
Generates:
  - Spatial comparison plots (Background / Target / Prediction / Error)
  - Vertical profile plots
  - Per-layer RMSE plots
  - Metrics bar chart
  - HTML report
"""
...
def create_comparison_plots(pred_ds, target_ds, bg_ds, save_dir, date_str,
                             depth_indices=None):
    ...
    for var_name, var_label in [('thetao', 'Temperature (°C)'), ('so', 'Salinity (PSU)')]:
        for depth_idx in depth_indices:
            ...
            im_err = axes[3].imshow(error_pred, cmap='RdBu_r',
            ...
            im_bg_err = axes[4].imshow(error_bg, cmap='RdBu_r',
                                       vmin=err_min, vmax=err_max,
                                       aspect='auto', origin='lower')
...
def create_vertical_profile_plots(pred_ds, target_ds, bg_ds, save_dir, date_str,
                                   profile_coords=None, depth_indices=None):
    """Create vertical profile plots at user-specified (lat, lon) locations."""
    ...
    for var_name, var_label in [('thetao', 'Temperature (°C)'), ('so', 'Salinity (PSU)')]:
        ...
        vmin, vmax = (0, 40) if var_name == 'thetao' else (30, 40)
        ...
        for i, (lat_val, lon_val, lat_idx, lon_idx) in enumerate(valid_points):
            ax = axes_flat[i + 1]
            pred_profile   = pred_ds[var_name].isel(
                time=0, latitude=lat_idx, longitude=lon_idx).values[depth_indices]
            target_profile = target_ds[var_name].isel(
                time=0, latitude=lat_idx, longitude=lon_idx).values[depth_indices]
            bg_profile     = bg_ds[var_name].isel(
                time=0, latitude=lat_idx, longitude=lon_idx).values[depth_indices]
...
def create_layer_rmse_plot(temp_layer_rmse, salt_layer_rmse, depth_values,
                            save_dir, date_str,
    ...
    _draw_layer_rmse(axes[0], temp_layer_rmse, depth_values,
                     'Temperature (°C)', '#e74c3c', 'Pred-Target RMSE')
    _draw_layer_rmse(axes[1], salt_layer_rmse, depth_values,
                     'Salinity (PSU)',   '#3498db', 'Pred-Target RMSE')
    ...
    if bg_temp_layer_rmse is not None and bg_salt_layer_rmse is not None:
        fig, axes = plt.subplots(1, 2, figsize=(14, 6))
        _draw_layer_rmse(axes[0], bg_temp_layer_rmse, depth_values,
                         'Temperature (°C)', '#27ae60', 'BG-Target RMSE')
        _draw_layer_rmse(axes[1], bg_salt_layer_rmse, depth_values,
                         'Salinity (PSU)',   '#f39c12', 'BG-Target RMSE')
...
def create_metrics_plot(metrics, temp_metrics, salt_metrics, save_dir, date_str):
    """Create a bar plot showing metrics."""
...
<head>
    ...
    <title>Ocean Reconstruction Results (Glory) - {date_formatted}</title>
    <style>
        body {{
        ...
        .metric-card h3 {{ margin: 0 0 15px 0; font-size: 1.2em; opacity: 0.9; }}
        .metric-value {{
            display: flex;
        ...
        .metric-number {{ font-weight: bold; font-size: 1.2em; }}
        .plot-grid {{ display: grid; grid-template-columns: 1fr; gap: 30px; margin-top: 20px; }}
        ...
        .plot-caption {{ margin-top: 10px; color: #666; font-style: italic; }}
        .info-box {{
            background: #e3f2fd;
...
<body>
    <div class="header">
        <h1>🌊 Ocean Reconstruction Results (Glory)</h1>
    ...
    <div class="section">
        ...
        <div class="info-box">
            <strong>Note:</strong> Each row shows Background (Glorys -7 days), Target, Prediction,
            Error (Pred - Target), and Error (BG - Target) at a given depth.
        </div>
        <div class="plot-grid">
"""

    # Spatial comparison plots (all except last 5: 2 profiles + 2 layer_rmse + 1 metrics)
    for plot_path in plot_rel_paths[:-5]:
...
"""
    ...
    <div class="section">
        ...
        <div class="info-box">
            <strong>Note:</strong> Vertical profiles show how temperature and salinity vary with depth
            at selected locations. Green dotted lines = background, blue lines = prediction,
            red dashed lines = ground truth.
        </div>
        <div class="plot-grid">
"""

    # Vertical profile plots (index -5 and -4)
    for plot_path in plot_rel_paths[-5:-3]:
        plot_name = os.path.basename(plot_path)
        html_content += f"""
            <div class="plot-container">
                <img src="{plot_path}" alt="{plot_name}">
...
"""
    ...
    <div class="section">
        <h2>📁 Output Files</h2>
        <table>
            <tr><th>File Type</th><th>File Path</th></tr>
            <tr><td>Prediction NetCDF</td><td>{os.path.basename(pred_path)}</td></tr>
            <tr><td>Target NetCDF</td><td>{os.path.basename(target_path)}</td></tr>
            <tr><td>HTML Report</td><td>report_{date_str}.html</td></tr>
        </table>
    </div>

    <div class="footer">
        <p>Generated by Ocean Reconstruction Model Inference System (Glory mode)</p>
...
def visualize_glory_results(pred_path, target_path, bg_path,
    ...
    """
    print("\n" + "=" * 60)
    print("Creating Glory-mode visualizations...")
    print("=" * 60)
    ...
    comparison_plots = create_comparison_plots(
        pred_ds, target_ds, bg_ds, save_dir, date_str, depth_indices)

    print("\n2. Creating vertical profile plots...")
    profile_plots = create_vertical_profile_plots(
        pred_ds, target_ds, bg_ds, save_dir, date_str, profile_coords, depth_indices)
