...
if __name__ == '__main__':
    try:
        ...
        print(f"input_mean.shape:{input_mean.shape}")
        ...
        # model_path = r"./checkpoints\recon\recon_model.onnx"
        ...
        if len(ort_outs) == 1:
            # Single output (our trained model)
            output = ort_outs[0].squeeze(0)
        elif len(ort_outs) >= 3:
            # Multiple outputs (original model format)
            output = ort_outs[2].squeeze(0)
        else:
            raise ValueError(f"Unexpected number of model outputs: {len(ort_outs)}")
